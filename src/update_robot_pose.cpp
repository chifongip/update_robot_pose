...
public: 
    resetPose(): tf2_buffer(), tf2_listener(tf2_buffer) 
    {
        ...
        reset_buf = 1;  // used for check the tag detections were stable or not 
        cnt_buf = 0;    // if the difference between current frame and previous frame is smaller than threshold, count++
        ...
        // construct transformation of usb_cam_link w.r.t base_link
        ...
        // construct transformation of base_link w.r.t usb_cam_link
        usb_cam_link_base_link_g = base_link_usb_cam_link_g.inverse(); 

        // initialize subscriber and publisher
        // the queue size of subscriber set to 10
...
private:
    ...
    void poseCallback(const apriltag_ros::AprilTagDetectionArray::ConstPtr& msg)
    {
        ...
        // true if tags were detected, and current linear and angular velocity were smaller than threshold 
